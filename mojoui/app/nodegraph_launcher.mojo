...
def comfy_nodegraph_launch_command() -> String:
    ...
    return (
        String("cd ")
        + String(MOJOUI_ROOT)
        + String(" && if ! pgrep -f '[/]tmp/mojoui_m6_nodegraph' >/dev/null && mkdir /tmp/mojoui_m6_nodegraph.launch.lock 2>/dev/null; then (trap 'rmdir /tmp/mojoui_m6_nodegraph.launch.lock' EXIT; (test -x ")
        + String(NODEGRAPH_BIN)
        + String(" || ")
        + String(PIXI_BIN)
        + String(" run mojo build -I . -Xlinker -L. -Xlinker -lmojoui_floor -Xlinker -lm examples/m6_nodegraph.mojo -o ")
        + String(NODEGRAPH_BIN)
        + String(") && setsid env LD_LIBRARY_PATH=")
        + String(MOJOUI_ROOT)
        + String(" ")
        + String(NODEGRAPH_BIN)
        + String(") > ")
